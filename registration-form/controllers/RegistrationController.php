...
class RegistrationController extends Controller
{
        ...
        return [
			...
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index','view','create','update','delete','mail','view-student-id','view-payment-receipt'],
						'roles' => ['@'],
					],
					[
						'allow' => true,
    ...
    {
    throw new UnauthorizedHttpException("Registration is now closed."); //Esta linea se debe eliminar para poder agregar registros nuevos
        // $registration = new Registration(['scenario'=>'Create']);
		$registration = new Registration();
		...
		if ($registration->load(Yii::$app->request->post())) {
			...
			if($registration->save())
			{
				Yii::$app->mailer->compose('registration/view-mail', ['model'=>$registration])
					...
					->setSubject('Registration Confirmation - ISICS 2018')
					->send();
		...
		if ($registration->load(Yii::$app->request->post())) {
			...
			if($valid)
			{
				if($registration->save())
				{
					...
					if($isSaved)
					{
						$mailSubject = 'Registration Confirmation Modified - ISICS 2018';
						if( empty( $registration->paid_by_credit_card ) && empty($registration->payment_receipt) && empty($registration->registrationCode) )
							$mailSubject = 'Pending Registration Modified - ISICS 2018';
